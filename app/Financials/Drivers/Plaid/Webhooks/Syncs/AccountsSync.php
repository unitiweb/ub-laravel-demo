...
use App\Financials\FinancialSyncInterface;
 ...
 */
class AccountsSync extends Sync implements FinancialSyncInterface
{
//    /**
//     * @return bool
...
//
//        // Get the institution id from the first account
//        $institutionId = $accounts->first()->institutionId ?? null;
//
//        // Upsert the institution
//        $institutionDetail = $this->upsertInstitutionDetail($institutionId);
//
...
//    }
    ...
    protected function upsertAccounts(Collection $accounts, DbCollection $bankAccounts, ?BankInstitution $bankInstitution): void
    {
        foreach ($accounts as $account) {
            if (!$bankAccount = $this->bankAccountExists($account->accountId, $bankAccounts)) {
            ...
            $bankAccount->officialName = $account->officialName;
     ...
     * @throws GuzzleException
     */
    protected function upsertInstitutionDetail(?string $institutionId): ?BankInstitutionDetail
    {
        // If the institution is already in the database then stop upsert
        if ($institutionDetail = BankInstitutionDetail::where('institutionId', $institutionId)->first()) {
